      ...
      return {
        ...
        to({ context, absoluteFilename }) {
          let name = path
            ...
            .replace(
              new RegExp(`(\.${platform}\.json)$`),
              ".json",
            );

          if (source) {
            name = name.replace(
              new RegExp(`(\.${source}\.json)$`),
              ".json",
            )
          }
    ...
    Object.entries(jsonData.usingComponents).forEach(
      ([componentName, pathUrl]) => {
        ...
        if (!platformPathUrl || dependents.length === 0) {
          tmp[platform === "aliapp" ? componentName.toLowerCase() : componentName] = pathUrl;
          return;
        }

        tmp[platform === "aliapp" ? componentName.toLowerCase() : componentName] = entry[platformPathUrl].includes("node_modules")
          ? `/${pathUrl}`
          : pathUrl;
      },
    );
