...
const hasSourceDefaultConfig = source ? glob.sync(`default.${source}.*(yaml|yml)`, globConfig).length > 0 : false;
...
const hasSourceEnvConfig = source ? glob.sync(`${PRJ_ENV}.${source}.*(yaml|yml)`, globConfig).length > 0 : false;
